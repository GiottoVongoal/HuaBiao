...
public class StringUtil {
    ...
    public static String getShowNumber(int number) {
        if (number > 1000) {
            return number / 10000 + "千";
        } else {
     ...
     * @return
     */
    public static int getTotalBytes(String in) {
        if (StringUtil.isEmpty(in)) {
            return 0;
        }
        try {
    ...
    public static String formatNumber(int number) {
        ...
        if (number >= 10000) {
            ...
            numberStr = String.valueOf(decimalFormat.format(((double) number) / 10000));
            return numberStr + "万";
        } else if (number >= 1000) {
            return numberStr.substring(0, 1) + "," + numberStr.substring(1, numberStr.length());
        } else {
            return numberStr;
        }
    }
     ...
     * @return
     */
    public static String formatGiftAlias(int number) {
     ...
     * @param content
     */
    public static void copyMsg(Context context, String content) {
        if (content == null)
            return;
        ...
        if (sdk < 11) {
            ...
            android.content.ClipData clip = android.content.ClipData.newPlainText("", content.trim());
            clipboard.setPrimaryClip(clip);
        }

        Toast.makeText(context, content + "已复制到剪贴板", Toast.LENGTH_SHORT).show();
    }
    ...
    public static String processMoney(String m) {
        if (TextUtils.isEmpty(m))
            return "0";
        Integer len = m.length();
        String money = "0";
        if (len > 12) {// 1猪=1千亿
            money = new BigDecimal(m).divide(new BigDecimal("100000000000"), 2, BigDecimal.ROUND_HALF_EVEN).toString() + "猪";
        } else if (len > 8) {
            money = new BigDecimal(m).divide(new BigDecimal("100000000"), 2, BigDecimal.ROUND_HALF_EVEN).toString() + "亿";
        } else if (len > 4) {
            money = new BigDecimal(m).divide(new BigDecimal("10000"), 2, BigDecimal.ROUND_HALF_EVEN).toString() + "万";
        } else {
            money = m;
        }
    ...
    public static String processCount(String count) {
        if (TextUtils.isEmpty(count))
            return "0";
        Integer len = count.length();
        String cnt = "0";
        if (len > 12) {// 1猪=1千亿
            cnt = new BigDecimal(count).divide(new BigDecimal("100000000000"), 1, BigDecimal.ROUND_HALF_EVEN).toString() + "z";
        } else if (len > 8) {
            cnt = new BigDecimal(count).divide(new BigDecimal("100000000"), 1, BigDecimal.ROUND_HALF_EVEN).toString() + "y";
        } else if (len > 4) {
            cnt = new BigDecimal(count).divide(new BigDecimal("10000"), 1, BigDecimal.ROUND_HALF_EVEN).toString() + "w";
        } else {
            cnt = count;
        }
